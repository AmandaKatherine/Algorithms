def frac_knap(csv9, capacity):
    ...
    while capacity > 0 and i > 0:
        ...
        if weight[i] < capacity:
            ...
            fractional = capacity/weight[i]
            items.append(value[i] * fractional)
            knapsack.append(name[i])
            capacity -= (weight[i] * fractional)
            total_weight += (weight[i] * fractional)
            del value[i]
            del weight[i]
            del name[i]
...
print(frac_knap('Assignment9.csv', 550))   
